...
- 2026-05-17: Added Unity visual-fix sprint foundations: placeholder-only road prototype mode, projection presets, debug overlay, horizon fade, shoulder strips, bridge segment gating, and manifest-driven review asset extraction with QA report.
- 2026-05-17: Verified the Unity Road Prototype placeholder-first workflow in the editor: `Create Or Refresh Scene` now creates a placeholder-only scene, imported assets use a separate preview scene, and Game view checks confirmed the portrait road/camera/car anchor before enabling real art layers.
- 2026-05-17: Added a RoadOnlyTest Unity review gate for the new road surface pack: imported only the seamless asphalt tile and double-yellow lane strip, locked their point/repeat/uncompressed import settings, created a road/lane-only scene, and captured still, 10-second motion, horizon lane, and near-road close-up screenshots before any background, car, signs, props, shoulder, cracks, weather, or vegetation are allowed back in.
- 2026-05-17: Added PixelLab MCP as a Codex remote server using `PIXELLAB_API_TOKEN` and created `docs/art/pixellab-roadside-map-object-queue-v1.md` for transparent-background roadside map-object generation based on the current Big Sur sign, vegetation, rock, guardrail, and marker reference sheets.
- 2026-05-18: Verified PixelLab MCP authentication, generated and downloaded three trial transparent roadside map objects into `apps/unity-client/Assets/NewTrip/Art/PixelLabDrafts/`, and recorded the first QA lesson: PixelLab is good for vegetation/object silhouettes, while readable sign text should be added through Unity text/overlay rather than baked generation.
- 2026-05-18: Generated five more PixelLab roadside drafts, confirming blank sign bodies, rocks, and wooden arrows are good candidates for manifest review while long guardrail prompts can accidentally create scenic background strips and should be regenerated with stricter no-background wording.
- 2026-05-18: Added a research-backed Unity pseudo-3D art/background/asset-pipeline knowledge base for motion-first road naturalness, Big Sur sunset layer requirements, road/lane material QA, sprite anchors, horizon bands, and mobile performance gates.
- 2026-05-18: Added a source-backed Unity mobile pixel road rendering knowledge base covering texture import settings, pixel-art stability, Sprite Atlases, batching/material policy, pseudo-3D road motion, sprite spawner pooling, overdraw, alpha overlays, and RoadOnlyTest acceptance risks.
- 2026-05-18: Added a focused pseudo-3D motion naturalness knowledge base for optical flow, shared road motion state, lane cadence, roadside object speed cues, car anchor stability, 10-second review gates, and conveyor-belt anti-patterns.
- 2026-05-18: Added the master NewTrip pseudo-3D driving knowledge base entrypoint, consolidating renderer, motion, art/background, asset extraction, and Unity mobile performance research into a symptom-to-fix workflow for the next Unity sprint.
- 2026-05-18: Implemented the first Unity motion-system sprint: shared `RoadMotionState`, depth-aware road/lane/shoulder motion, road-relative lane width, distance-driven placeholder roadside markers, safer Big Sur projection defaults, and a Unity-generated 10-second RoadMotion capture matrix.
- 2026-05-18: Rewired RoadOnlyTest to use the user's attached road material sources directly: asphalt as the base road texture, crack texture as a subtle road detail overlay, worn yellow texture as a restrained lane-wear overlay, and regenerated Unity road-only screenshots for review.
- 2026-05-18: Fixed the RoadOnlyTest renderer integration for the user's road material pass: verified actual texture loading in Unity logs, locked point/repeat/uncompressed/no-mipmap import settings, changed asphalt UVs to road-width-based repeat instead of full-width stretching, tapered lane width toward the horizon, and regenerated road-only still/motion/close-up captures.
- 2026-05-18: Upgraded the RoadOnlyTest road material pass again: generated a low-shimmer seamless runtime asphalt tile from the user's approved source style, replaced the central lane strip with two independent projected yellow-line meshes, aligned RoadOnly/MotionReview/RoadPrototype loading to the runtime asphalt and single-line yellow assets, and regenerated Unity road-only still, 10-second motion, horizon-lane, and near-road close-up captures.
- 2026-05-18: Added the RoadOnlyTest V2 A/B review pass: captured viewport-depth yellow lines versus road-relative yellow lines, added left/right white road-edge lines before any crack/background/car layers, kept crack overlay disabled, and made the screenshot capture path auto-refresh stale RoadOnly scenes before export.
- 2026-05-18: Accepted RoadOnly B as the default runtime lane style: future Unity road composites should use the wider road-relative double-yellow line preset, with the viewport-depth A pass kept only as historical comparison.
- 2026-05-18: Locked the Step 1 Unity driving angle contract before rebuilding scenery: 9:16 portrait frame, `5.625 x 10.0` world units, `road_horizon_y = 0.52`, `car_anchor_y = 0.105`, and `HUD_safe_top_y = 0.86+` are now centralized for road, car, debug overlay, camera, and future background alignment.
- 2026-05-18: Added the Step 2 SkyOnlyTest gate: imported the approved 9:16 sunset ocean sky and created a Unity scene/menu path that renders only an opaque full-screen `SkyLayer` at sorting order 0, with no road, car, foreground, bridge, UI, signs, weather, or movement.
- 2026-05-18: Added the Step 3 SkyFarRoadTest gate: imported the transparent `far_coastal_mountains_01` horizon band, created a Unity menu path that layers it between the approved sky and RoadOnly B road, and locked the far layer to bottom-center alignment just above `road_horizon_y = 0.52` with no car, UI, bridge, foreground props, signs, or weather.
- 2026-05-18: Added the Step 4 road color tuning pass: base road rendering now uses an opaque road shader with named Big Sur sunset tint presets, keeps horizon blending out of road alpha, warms lane/edge colors, adds a separate `HorizonHazeLayer` for road-apex softness, and adds a Unity capture menu for Sky + Far + Road A/B/C color screenshots before any car, UI, props, bridge, or weather return.
- 2026-05-19: Formalized Step 4 Horizon Haze as its own Unity review gate: generated the deterministic transparent `horizon_haze_warm_v01.png` overlay, locked haze sorting between opaque road and lane/edge markings, and added 9:16 alpha/debug/close-up capture outputs for review before adding car, UI, bridge, guardrails, signs, props, weather, or vegetation.
- 2026-05-19: Replaced the Step 4 Horizon Haze source with the user-provided transparent PNG pack, rebuilt `SkyFarRoadTest` with an inspector-tunable `HorizonHazeLayerController`, and regenerated the 9:16 alpha/debug/close-up review captures without changing camera, road geometry, sky, far background, car, UI, bridge, props, signs, or weather.
- 2026-05-19: Strengthened the Step 4 Horizon Haze review default after the first user pass read too faint: `SkyFarRoadTest` now uses the user-provided haze sprite at `alpha = 0.24`, `hazePositionY = road_horizon_y + 0.028`, `scale = 3.4 x 0.55`, and keeps A/B/C captures at `0.16 / 0.24 / 0.30` for visible but still atmospheric road-apex blending.
- 2026-05-19: Completed the Step 5 Unity Road Lock Pass: documented the locked procedural road contract, added a `Capture Step 5 Road Lock Pass` Unity menu/report path, regenerated RoadOnly still/10-second/horizon/bottom captures and Sky/Far/Road/Haze captures through Unity MCP, and locked the accepted road stack to code-generated road mesh, opaque asphalt tile, RoadOnly B road-relative double-yellow lines, projected white edge lines, shared `RoadMotionState.visualDistanceMeters`, and horizon haze rather than full-road imagery.
- 2026-05-20: Added the Step 6 Unity CarAnchorTest gate: the accepted Step 5 Sky/Far/Road/HorizonHaze stack now gets only a bottom-center `car_rear_view_clean` rear sprite at `car_anchor_y = 0.105`, a subtle contact shadow, A/B/C scale captures, a 10-second road-motion capture under a fixed car, and debug/contact close-ups; this pass keeps UI, bridge, guardrails, signs, roadside props, weather, shoulders, vegetation, and full-road images out of scope.
- 2026-05-20: Replaced the Step 6 car source with the user-provided no-white-edge `unity_car_rear_png_assets_package` sprite, imported it as `car_rear_view_clean.png` with the manifest bottom-center pivot `(0.5, 0.003384)`, and recalibrated the accepted car scale from `1.06` to `0.56` so the cleaner cropped sprite preserves the previous visual footprint.
- 2026-05-21: Added durable Codex thread profiles for Unity Visual QA, Day 14 demo stabilization, and backend spine/idempotency work, and installed project-specific `newtrip-visual-gate` and `newtrip-v1-slice` skills for repeated gate and vertical-slice workflows.
- 2026-05-21: Applied the Gemini pixel-art rendering guidance to the Unity visual QA path: added a global NewTrip art import rule for Point/no-mipmap/uncompressed textures with 256 PPU Sprite assets, added a dedicated `NewTrip/PixelUnlitTransparent` material path for sprite layers/spawners, kept the road on an opaque Unlit-style shader, added `RoadProjectionPreset.GeminiLowCamera` as a review-only low-camera comparison beside the accepted RoadOnly B contract, and regenerated RoadOnly B plus Gemini review captures through Unity MCP.
- 2026-05-21: Hardened the Gemini pixel-art rules in Unity: runtime prototype SpriteRenderers now use the shared Unlit transparent material, all generated/loaded sprite assets use the shared 256 PPU constant, road prototype refresh defaults to the Gemini low-camera projection candidate, RoadOnly B legacy captures now exercise that projection, and Unity MCP confirmed point/repeat/uncompressed road textures plus the opaque road shader.
- 2026-05-21: Tightened the Step 6 car grounding pass: `CarRearController` now follows the strict `PlayerCarRoot -> ContactShadow + CarBody` hierarchy, keeps the root anchor fixed, applies absolute-sine engine bob only to `CarBody.localPosition.y`, inversely scales/fades the contact shadow, and regenerated CarAnchorTest review captures through Unity MCP.
- 2026-05-21: Added the Step 6B car-road integration fix: kept Step 5 road geometry locked, compressed only the car body perspective fit, moved car suspension bob to shared `RoadMotionState.visualDistanceMeters`, lowered bob speed/amplitude with startup easing, added subtle wheel speed cues, and extended CarAnchorTest captures to slow/cruise/boost plus startup review frames.
- 2026-05-22: Replaced the Unity visual-gate sky source with the user-provided soft orange 9:16 background as `sky_step2_soft_orange_background.png`, rewired SkyOnlyTest, SkyFarRoadTest, and imported RoadPrototype mode to use it, then regenerated SkyOnlyTest, SkyFarRoadTest, and Step 6 CarAnchorTest through Unity MCP for review.
- 2026-05-22: Retuned the Step 4 HorizonHazeLayer after the first visible pass still read too faint, added the user-provided orange orb as a separate `SunLayer`, replaced Step 6 CarAnchorTest with the user-provided rear car and soft ground shadow sprites, and regenerated SkyFarRoadTest plus CarAnchorTest captures through Unity MCP without changing the locked road geometry.
- 2026-05-22: Applied the Sky/Far/Road atmospheric integration fix: kept Step 5 road geometry locked, aligned the far background baseline to the current `road_horizon_y = 0.66`, added an opaque `BigSurSunsetAtmosphericBlend` road color-wash preset, retuned the user-provided HorizonHazeLayer to a subtler `0.30` review default, and regenerated HorizonHaze plus Step 5 Road Lock captures through Unity MCP.
- 2026-05-22: Completed the Step 6B car-road grounding and speed-match review: kept the road geometry locked, moved visual speed smoothing into `RoadMotionState`, accepted the heavier `0.51` car scale with compressed rear-body perspective, slowed startup/bob motion, kept the tire baseline fixed at `car_anchor_y = 0.105`, added subtle wheel/shadow speed cues, and regenerated slow/cruise/boost CarAnchorTest captures through Unity MCP.
- 2026-05-22: Prepared the Windows/Codex Unity visual QA handoff, documented the current accepted Step 5/Step 6B state, recorded the next Road Perspective Review Pass candidates, and ignored local-only Unity screenshots/recovery folders before pushing the workspace.
- 2026-05-22: Added the Road Perspective Review Pass tooling: review-only `ReferenceGentleRoad` and `LongCoastRoad` projection presets, a separate `RoadPerspectiveReview` Unity scene/capture menu, A/B/C still/motion/close-up report outputs, and a contract doc so road angle can be judged without changing material, car, UI, scenery, or the accepted Road Lock production scene.
- 2026-05-22: Accepted Road Perspective Review candidate B as the active road angle: `RoadProjectionPreset.ReferenceGentleRoad` is now the `RoadViewportContract` default with `road_horizon_y = 0.60`, `bottom_y = -0.06`, `near_half_width = 0.86`, `horizon_half_width = 0.014`, and `depth_curve = 2.05`; the previous Gemini low-camera projection remains only as historical A/B comparison.
- 2026-05-22: Started the Side Objects deployment pass with the first user-provided low wooden guardrail asset: `SideObjectSpawner` now projects objects through `Pseudo3DRoadRenderer.Sample(depthT)`, spawns at `depthT = 1.0`, despawns at `0.0`, scales with `baseScale * (1 - pow(depthT, 2.45))`, sorts with `round((1 - depthT) * 1000)`, compensates bottom-center pivots at runtime, and adds a dedicated SideObject Guardrail Review scene/capture menu before trees, signs, dirt patches, UI, bridge, or weather return.
